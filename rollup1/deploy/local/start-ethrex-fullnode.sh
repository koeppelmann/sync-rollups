...
if [ -f "$ROLLUP_DIR/deploy/local/.env" ]; then
    source "$ROLLUP_DIR/deploy/local/.env"
fi
...
DATADIR="${DATADIR:-$ROLLUP_DIR/state/ethrex-fullnode}"
LOG_DIR="${LOG_DIR:-$ROLLUP_DIR/logs}"
...
(cd "$ROLLUP_DIR/ethrex-fullnode" && cargo build --release 2>&1 | tail -3)

FULLNODE_BIN="$ROLLUP_DIR/ethrex-fullnode/target/release/sync-rollups-ethrex-fullnode"
