  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case "--rollups":
        ...
        break;
      case "--rollup-id":
        config.rollupId = parseInt(args[++i]);
        break;
    }
  }
 ...
 */
async function computeProxyAddress(l2TargetAddress: string, rollupId: number): Promise<string> {
  const rollupsContract = new ethers.Contract(config.rollupsAddress, [
    "function computeCrossChainProxyAddress(address,uint256,uint256) view returns (address)"
  ], provider);
  // domain = L1 chain ID; get it from the provider
  const network = await provider.getNetwork();
  return await rollupsContract.computeCrossChainProxyAddress(
    l2TargetAddress, rollupId, network.chainId
  );
}
...
async function handleRequest(
  ...
  if (url.pathname === "/register-hint" && req.method === "POST") {
    ...
    // Accept {l2TargetAddress, rollupId} — proxy address is derived deterministically
    // Also accept legacy {proxyAddress, l2TargetAddress} for backward compat
    const l2TargetAddress = parsed.l2TargetAddress;
    const rollupId = parsed.rollupId ?? config.rollupId;

    if (!l2TargetAddress) {
      res.writeHead(400, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: "Missing l2TargetAddress" }));
      return;
    }

    try {
      const proxyAddress = parsed.proxyAddress || await computeProxyAddress(l2TargetAddress, rollupId);
      registerHint(proxyAddress, l2TargetAddress);
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ success: true, proxyAddress }));
    } catch (e: any) {
