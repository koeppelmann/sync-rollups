...
When an L1 address sends a transaction to a `CrossChainProxy` contract (deployed via `Rollups.createCrossChainProxy`), the proxy forwards it to `Rollups.executeCrossChainCall`. The L1 contract looks up a pre-loaded execution entry, applies state deltas, and emits `L2ExecutionPerformed`. On L2, the fullnode replays this as an operator system call to the target address with the original calldata and value.
...
### Rule 2: Coinbase = `msg.sender` of the L1 caller

The `coinbase` (block beneficiary / fee recipient) of each L2 block is set to the `msg.sender` of whoever called the L1 function that triggered the state change:

- For `executeL2TX`: the address that called `executeL2TX` on L1
- For cross-chain proxy calls: the address that sent the transaction to the `CrossChainProxy` on L1
- For `postBatch`: the address that called `postBatch` on L1

This makes the coinbase deterministic and verifiable from L1 data alone.

> **Implementation note (current limitation):** Both the builder simulation and event processor currently use `coinbase=0x0` (zero address). Changing the coinbase affects the state root in reth even with 0 gas fees, because the fee recipient account is "touched" during block processing. To implement spec-compliant coinbase, both the builder simulation and event processor must use the same coinbase value, which requires passing the expected L1 caller through the execution plan.
...
System transactions (proxy deployments, execution table loading, cross-chain calls) use:
- `maxFeePerGas` = the expected next block base fee, computed deterministically from the parent block header using the EIP-1559 formula
- `maxPriorityFeePerGas` = 0 (no tip)

This ensures system transaction encoding is identical across all fullnodes (same `maxFeePerGas` → same tx hash → same state root). The operator account is pre-funded with sufficient balance to cover gas costs.
