...
This document specifies exactly what an alternative L2 execution engine must implement to be compatible with the sync-rollups fullnode. It is derived from the current reth-based implementation in `tooling/fullnode/state-manager.ts` and `tooling/fullnode/event-processor.ts`.
...
**Parameters:**
- `timestamp`: `options.timestamp` if provided, otherwise `headTimestamp + 1`. Must be strictly greater than `headTimestamp`.
...
2. Get L2 chain ID: eth_chainId()

3. Ensure source proxy is deployed on L2:
   a. Compute expected proxy address:
      eth_call({
        to: CrossChainManagerL2_address,
        data: computeCrossChainProxyAddress(sourceAddress, rollupId, l2ChainId)
      })
   b. Check if proxy exists: eth_getCode(proxyAddress)
   c. If no code, deploy via sendSystemTx:
      sendSystemTx(
        CrossChainManagerL2_address,
        createCrossChainProxy(sourceAddress, rollupId)
      )
      // This tx goes to txpool, NOT mined yet

4. Send the main call to txpool (deferred mining):
   sendSystemTx(l2Target, callData, value)

5. mineBlock()
   // ONE block containing: [proxy deployment (if needed), main call]

6. Verify: eth_getBlockByNumber("latest").stateRoot == newState from event
7. Update tracked state
```

**Key constraint:** All preparation transactions (proxy deployment) and the main call MUST be in a single L2 block. This is achieved by sending all transactions to the txpool without mining, then calling `mineBlock()` once.
...
### Rule 2: Coinbase
**Spec:** `coinbase = msg.sender` of the L1 transaction.
**Current implementation:** `coinbase = 0x0000000000000000000000000000000000000000` (both builder and event processor use default). Changing coinbase affects state root because the fee recipient account is "touched" during block processing and receives priority fees.

### Rule 3: Timestamp
**Spec:** `timestamp = L1 block timestamp`.
**Current implementation:** `timestamp = headTimestamp + 1` (both builder and event processor use default). The builder cannot predict the L1 block timestamp at simulation time.
...
### Rule 6: Zero gas pricing
The L2 chain uses standard EIP-1559 gas pricing. The genesis `baseFeePerGas` is 1 gwei (`0x3B9ACA00`). System transactions set `maxFeePerGas` to the expected next block base fee (computed deterministically from the parent block) and `maxPriorityFeePerGas = 0`. User transactions must set `maxFeePerGas ≥ baseFee`.
