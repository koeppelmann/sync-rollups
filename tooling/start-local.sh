...
docker compose -f blockscout/l1/docker-compose.yml down -v 2>/dev/null || true
docker compose -f blockscout/l2/docker-compose.yml down -v 2>/dev/null || true
sleep 2
...
# Build all contracts (core + tooling) so artifacts are available
echo "  Compiling contracts..."
cd ..
forge build > /dev/null 2>&1
forge build --contracts tooling/contracts --out out > /dev/null 2>&1
cd tooling
...
if [ -f "$ETHREX_BIN" ]; then
  ...
  ETHREX_FULLNODE_BIN="./ethrex-fullnode/target/release/sync-rollups-ethrex-fullnode"
...
docker compose -f blockscout/l1/docker-compose.yml up -d 2>/dev/null
docker compose -f blockscout/l2/docker-compose.yml up -d 2>/dev/null
...
OPERATOR_ADDR=$(node -e "const {Wallet,solidityPackedKeccak256}=require('ethers'); const k=solidityPackedKeccak256(['string','address','uint256','uint256'],['sync-rollups-operator','$ROLLUPS_ADDR',0,31337]); console.log(new Wallet(k).address);")
echo "  Operator address: $OPERATOR_ADDR"

# Symlink tooling contracts into src/ for forge verification
ln -sf ../tooling/contracts/Counter.sol ../src/Counter.sol
ln -sf ../tooling/contracts/Logger.sol ../src/Logger.sol
